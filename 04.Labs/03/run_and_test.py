...
def run_and_test(magic_arguments, outputs_inputs):
    ...
    if magic_arguments.startswith('-c'):
        ...
        match = re.search(r'from (\S+) import \*', statements)
        if match:
    ...
    if '-e' in options:
        ...
        print(error)
        return
    elif len(p.stdout[slice(start, end)]) > size_limit:
...
def process_possible_option(magic_arguments, options):
    ...
    if prefix[1] == 'e':
        ...
        return 2
    if prefix[1] in 'ts':
        match = re.compile(r'\s*\d+').match(magic_arguments, 2)
        try:
