...
user_input = turtle.textinput('Hypotrochoid or Epitrochoid?',
                              'Provide no input for an Epitrochoid, '
                              'and any input for a Hypotrochoid'
                             )
...
R = round(R)
# max_r defined in such a way that centre of rolling circle
# is at most dim away from origin.
min_r, max_r = 10, dim - direction * R
r = turtle.numinput('Rolling circle',
                    f'Radius r between {min_r} and {max_r}: ',
                    minval=min_r, maxval=max_r
                   )
r = round(r)
# d defined in such a way that drawing point is at most dim away from origin.
min_d, max_d = (0, dim - R - direction * r) if direction == 1 or r < R\
                                            else (0, dim - r + R)
d = turtle.numinput('Point', 'Distance d to centre of rolling circle '
                             f'between {min_d} and {max_d}: ',
                    minval=min_d, maxval=max_d
                   )
d = round(d)
period = r // gcd(r, R)
...
turtle.begin_fill()
if d == 0:
    # It is a circle, we draw it only once rather than period
    # many times.
    period = 1
for i in range(0, 360 * period + 1):
    theta = radians(i)
    psi = b * theta
    turtle.goto(a * cos(theta) - direction * d * cos(psi),
                a * sin(theta) - d * sin(psi)
               )
turtle.end_fill()
turtle.listen()
# Press S after trochoid has been drawn to save it as a pdf.
turtle.onkey(lambda trochoid=trochoid, R=R, r=r, d=d: save(trochoid, R, r, d),
             'S'
            )
turtle.mainloop()
