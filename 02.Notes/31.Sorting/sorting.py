...
class SortingExperimenter(tk.Tk):
    def __init__(self):
        ...
        possible_sorting_algorithms = 'Bubblesort', 'Selectionsort',\
    ...
    def boldface_help(self):
        tkinter.messagebox.showinfo(
                'Boldface',
                'When two numbers are being compared, they become '
                'boldface.'
                                   )
    ...
    def blue_help(self):
        tkinter.messagebox.showinfo(
                ...
                'iterative algorithms, all numbers are blue unless '
                'their current status is special and they receive a '
                'different colour.\n\nFor recursive algorithms '
                '(Quicksort, Mergesort), the numbers in blue (together '
                'with or excepted those whose status is special and '
                'which receive a different colour) are those involved '
    ...
    def red_help(self):
        tkinter.messagebox.showinfo(
                'Red',
                'For all algorithms except Mergesort, two numbers are '
                'coloured red when they are about to be swapped.\n\n'
                'For Mergesort, a number is coloured in red when it is '
                'about to be copied to a location indicated by a star.'
                                   )
    ...
    def yellow_help(self):
        tkinter.messagebox.showinfo(
                'Yellow',
                'Yellow is used:\n'
                ' - when a number is being compared with another '
                'number (so its font is boldface), will not be '
                'swapped, and is not known to be in place,\n'
                ' - or when a number has been copied from somewhere '
                'else (which except for Mergesort, means that it has '
                'been swapped with another number) and is not known to '
    ...
    def green_help(self):
        tkinter.messagebox.showinfo(
                'Green',
                'Green is the colour of the numbers that have been put '
                'in place.\n\nFor Insertionsort and Shellsort, this '
                'happens at the very end.\n\nFor Bubblesort, this can '
                'happen earlier.\n\nFor all other algorithms, this has '
                'to happen earlier for some numbers.'
                                   )
    ...
    def set_user_input_interface(self):
        ...
        self.input_button = tk.Button(
                    self.user_input_interface, text='Set input',
    ...
    def set_control_interface(self, selected_sorting_algorithm):
        ...
        self.algorithm_button = tk.Button(
                                    control_buttons, width=14,
                                    text='Start ' + selected_sorting_algorithm,
                                    command=self.start_or_stop_sorting
                                         )
        self.algorithm_button.pack(padx=80, side=tk.LEFT)
        self.step_button = tk.Button(control_buttons, text='Step',
                                     state=tk.DISABLED, command=self.step
                                    )
        self.step_button.pack(padx=80)
        control_buttons.pack()

    def update_selected_algorithm_and_interfaces(self,
                                                 selected_sorting_algorithm
                                                ):
    ...
    def update_interfaces(self, selected_sorting_algorithm):
        if self.tracing:
            self.algorithm_button.config(
                            text='Stop ' + selected_sorting_algorithm
                                        )
    ...
    def start_or_stop_sorting(self):
        ...
        if self.tracing:
            ...
            if self.selected_sorting_algorithm == 'Mergesort':
            ...
            if self.selected_sorting_algorithm == 'Batchersort':
                # The algorithm assumes that the length of the list to
                # sort is a power of 2
                largest_power_of_two_at_most_equal_to_length_of_list =\
                                     2 ** int(log(self.list_to_sort.length, 2))
                if largest_power_of_two_at_most_equal_to_length_of_list !=\
                                                      self.list_to_sort.length:
                    self.list_to_sort.display_list(
                           largest_power_of_two_at_most_equal_to_length_of_list
                                                  )
            for i in range(self.list_to_sort.length):
                self.list_to_sort.displayed_list.itemconfig(
                   self.list_to_sort.cells[i], fill=ListToSort.live_item_colour
                                                           )
            if self.selected_sorting_algorithm == 'Bubblesort':
                self.executing_algorithm =\
                                          self.list_to_sort.trace_bubble_sort()
            elif self.selected_sorting_algorithm == 'Selectionsort':
                self.executing_algorithm =\
                                       self.list_to_sort.trace_selection_sort()
            elif self.selected_sorting_algorithm == 'Insertionsort':
                self.executing_algorithm =\
                                       self.list_to_sort.trace_insertion_sort()
            elif self.selected_sorting_algorithm == 'Shellsort':
                self.executing_algorithm = self.list_to_sort.trace_shell_sort()
            elif self.selected_sorting_algorithm == 'Mergesort':
                self.executing_algorithm = self.list_to_sort.trace_merge_sort()
            elif self.selected_sorting_algorithm == 'Quicksort':
                self.executing_algorithm = self.list_to_sort.trace_quick_sort()
            elif self.selected_sorting_algorithm == 'Heapsort':
                self.executing_algorithm = self.list_to_sort.trace_heap_sort()
            elif self.selected_sorting_algorithm == 'Batchersort':
                self.executing_algorithm =\
                                         self.list_to_sort.trace_batcher_sort()
        else:
            self.list_to_sort.list = self.original_list
            for i in range(self.list_to_sort.length):
                self.list_to_sort.displayed_list.itemconfig(
                                     self.list_to_sort.cells[i],
...
class ListToSort(tk.Frame):
    ...
    def __init__(self):
        super(ListToSort, self).__init__(bd=10, padx=20, pady=20)
        self.displayed_list = tk.Canvas(self, width=20 * self.cell_size + 5,
                                        height=1.4 * self.cell_size + 5
                                       )
        self.displayed_list.pack()
        self.displayed_extra_list = tk.Canvas(self,
                                              width=20 * self.cell_size + 5,
                                              height=1.4 * self.cell_size + 5
                                             )
    ...
    def display_list(self, length):
        ...
        self.displayed_list.create_line(
                        self.offset, 5,
                        self.offset + self.length * self.cell_size, 5,
                        width=0.3, fill=self.line_colour
                                       )
        self.displayed_list.create_line(
                        self.offset, 5 + self.cell_size,
                        self.offset + self.length * self.cell_size,
                        5 + self.cell_size, width=0.3, fill=self.line_colour
                                       )
        for i in range(self.length + 1):
            self.displayed_list.create_line(
                        self.offset + i * self.cell_size, 5,
                        self.offset + i * self.cell_size, 5 + self.cell_size,
                        width=0.3, fill=self.line_colour
                                           )
        for i in range(self.length):
            self.cells[i] = self.displayed_list.create_text(
                        self.offset + (i + 0.5) * self.cell_size,
                        5 + self.cell_size / 2, text=self.list[i]
                                                           )
            self.displayed_list.create_text(
                        self.offset + (i + 0.5) * self.cell_size,
                        5 + self.cell_size * 1.2,
                        text=i, font=('normal', 7, 'normal')
                                           )

    def display_extra_list(self):
        self.offset = (20 - self.length) * self.cell_size / 2 + 5
        self.displayed_extra_list.create_line(
                        self.offset, 5,
                        self.offset + self.length * self.cell_size, 5,
                        width=0.3, fill=self.line_colour
                                             )
        ...
        while swapped and bound:
            ...
            for i in range(bound):
                if self.list[i] > self.list[i + 1]:
                    self.displayed_list.itemconfig(
                                self.cells[i], fill=self.moving_item_colour,
                                font=('normal', 0, 'bold')
                                                  )
                    self.displayed_list.itemconfig(
                                self.cells[i + 1],
                    ...
                    if i + 1 == bound:
                        self.displayed_list.itemconfig(
                                self.cells[i + 1], text=self.list[i + 1],
                                fill=self.in_place_item_colour,
                                font=('normal', 0, 'normal')
                                                      )
                    else:
                    ...
                    self.displayed_list.itemconfig(
                                self.cells[i], fill=self.nonmoving_item_colour,
                                font=('normal', 0, 'bold')
                                                  )
                    self.displayed_list.itemconfig(
                                self.cells[i + 1],
                    ...
                    yield
                    self.displayed_list.itemconfig(
                                self.cells[i], fill=self.live_item_colour,
                                font=('normal', 0, 'normal')
                                                  )
                    self.displayed_list.itemconfig(
                                self.cells[i + 1], font=('normal', 0, 'normal')
                                                  )
                    if i + 1 == bound:
                        for j in range(new_bound + 1, bound + 1):
                            self.displayed_list.itemconfig(
                                self.cells[j], fill=self.in_place_item_colour
                                                          )
                        if new_bound == 0:
                            self.displayed_list.itemconfig(
                                self.cells[0], fill=self.in_place_item_colour
                                                          )
                        yield
            bound = new_bound
    ...
    def trace_selection_sort(self):
        for i in range(self.length - 1):
            ...
            if index_of_min != i:
                ...
                yield
                self.list[i], self.list[index_of_min] =\
                                        self.list[index_of_min], self.list[i]
                self.displayed_list.itemconfig(self.cells[i],
                                               text=self.list[i],
                                               fill=self.in_place_item_colour
                                              )
                ...
                if i == self.length - 2 or self.length == 2:
                    self.displayed_list.itemconfig(
                              self.cells[i + 1], fill=self.in_place_item_colour
                                                  )
                yield
    ...
    def trace_insertion_sort(self):
        for i in range(1, self.length):
            ...
            while j and self.list[j - 1] > self.list[j]:
                self.displayed_list.itemconfig(self.cells[j - 1],
                                               fill=self.moving_item_colour,
                                               font=('normal', 0, 'bold')
                                              )
                self.displayed_list.itemconfig(self.cells[j],
                ...
                if j != 1 or i != self.length - 1:
                    self.displayed_list.itemconfig(
                                self.cells[j - 1], text=self.list[j - 1],
                                fill=self.nonmoving_item_colour,
                                font=('normal', 0, 'normal')
                                                  )
                    self.displayed_list.itemconfig(
                                self.cells[j], text=self.list[j],
                                fill=self.nonmoving_item_colour,
                                font=('normal', 0, 'normal')
                                                  )
                    yield
                    self.displayed_list.itemconfig(
                                self.cells[j], fill=self.live_item_colour
                                                  )
                else:
                    self.displayed_list.itemconfig(self.cells[j - 1],
                                                   text=self.list[j - 1],
            ...
            if j:
                ...
                for i in range(self.length):
                    self.displayed_list.itemconfig(
                                self.cells[i], fill=self.in_place_item_colour
                                                  )
                yield
                return
            else:
    ...
    def trace_shell_sort(self):
        for h in range(self.length - 1, 0, -1):
            # We use Pratt's method which uses as gaps all numbers of
            # the form 2^i * 3^j
            p = h
            while p % 2 == 0:
                p //= 2
            ...
            for i in range(self.length - h):
                ...
                self.displayed_list.itemconfig(self.cells[i + h],
                                               font=('normal', 0, 'bold')
                                              )
                if self.list[i + h] < self.list[i]:
                    ...
                    self.displayed_list.itemconfig(
                            self.cells[i + h], fill=self.moving_item_colour,
                                                  )
                    yield
                    self.list[i + h], self.list[i] =\
                                            self.list[i], self.list[i + h]
                    self.displayed_list.itemconfig(
                            self.cells[i], text=self.list[i],
                            font=('normal', 0, 'normal')
                                                  )
                    self.displayed_list.itemconfig(
                            self.cells[i + h], text=self.list[i + h],
                            font=('normal', 0, 'normal')
                                                  )
                    if h == 1:
                        self.displayed_list.itemconfig(
                    ...
                    if h == 1 and i == self.length - h - 1:
                        self.displayed_list.itemconfig(
                             self.cells[i + h], fill=self.in_place_item_colour
                                                      )
                    else:
                        self.displayed_list.itemconfig(
                             self.cells[i + h], fill=self.nonmoving_item_colour
                                                      )
                    yield
                    if h == 1 and i == self.length - h - 1:
                        return
    ...
    def trace_merge_sort(self):
        ...
        yield from self.trace_mergesort(self.list, self.extra_list, 0,
                                        self.length, 0
                                       )
    ...
    def trace_mergesort(self, L1, L2, start, end, depth):
        ...
        if end == start + 1:
            if depth == 0:
                self.displayed_list.itemconfig(self.cells[start],
                                               fill=self.in_place_item_colour
                                              )
                yield
            else:
        ...
        while i1 < half_length and i2 < end:
            if depth % 2:
                self.displayed_list.itemconfig(self.cells[i1],
                                               fill=self.nonmoving_item_colour,
                                               font=('normal', 0, 'bold')
                                              )
                self.displayed_list.itemconfig(self.cells[i2],
                                               fill=self.nonmoving_item_colour,
                                               font=('normal', 0, 'bold')
                                              )
                yield
            else:
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[i1], fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                                    )
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[i2], fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                                    )
                yield
            ...
            if depth % 2:
                self.displayed_list.itemconfig(self.cells[j1],
                                               fill=self.moving_item_colour,
                                               font=('normal', 0, 'normal')
                                              )
                self.displayed_list.itemconfig(self.cells[j2],
                                               fill=self.live_item_colour,
                                               font=('normal', 0, 'normal')
                                              )
                self.displayed_extra_list.itemconfig(self.extra_cells[i],
                ...
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[i], text=self.extra_list[i],
                        fill=self.nonmoving_item_colour
                                                    )
                yield
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[i], fill=self.live_item_colour
                                                    )
            else:
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[j1], fill=self.moving_item_colour,
                        font=('normal', 0, 'normal')
                                                    )
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[j2], fill=self.live_item_colour,
                        font=('normal', 0, 'normal')
                                                    )
                ...
                L1[i] = L2[j1]
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[j1], fill=self.live_item_colour
                                                    )
                if depth:
                    self.displayed_list.itemconfig(
                                      self.cells[i], text=self.list[i],
            ...
            if select_i1:
                ...
                i2 += 1
            i += 1
        yield from self.transfer_end_of_list(L1, L2, i, i1, half_length, depth)
        yield from self.transfer_end_of_list(L1, L2, i + half_length - i1, i2,
                                             end, depth
                                            )
        if depth:
            for i in range(start, end):
    ...
    def transfer_end_of_list(self, L1, L2, i, j, bound, depth):
        while j < bound:
            if depth % 2:
                self.displayed_list.itemconfig(self.cells[j],
                                               fill=self.moving_item_colour
                ...
                self.displayed_extra_list.itemconfig(
                        self.extra_cells[j], fill=self.moving_item_colour
                                                    )
                self.displayed_list.itemconfig(self.cells[i], text='*')
                yield
                L1[i] = L2[j]
                self.displayed_extra_list.itemconfig(self.extra_cells[j],
                                                     fill=self.live_item_colour
                                                    )
                if depth:
                    self.displayed_list.itemconfig(
    ...
    def _trace_quick_sort(self, start, last):
        ...
        yield from self.trace_partition(start, last)
        split_point = self.trace_partition.split_point
        ...
        if split_point != start:
            ...
            yield
        yield from self._trace_quick_sort(start, split_point - 1)
        if start < split_point < last:
            ...
            yield
        yield from self._trace_quick_sort(split_point + 1, last)

    def trace_partition(self, start, end):
        ...
        while True:
            ...
            while left_mark <= right_mark\
                  and self.list[left_mark] <= pivot_value:
                self.displayed_list.itemconfig(self.cells[left_mark],
                                               fill=self.nonmoving_item_colour,
                                               font=('normal', 0, 'bold')
                                              )
                yield
            ...
            while right_mark > left_mark\
                  and self.list[right_mark] >= pivot_value:
            ...
            if right_mark < left_mark:
                ListToSort.trace_partition.split_point = right_mark
                self.displayed_list.itemconfig(self.cells[start],
                                               font=('normal', 0, 'normal')
                                              )
            if right_mark == start:
            ...
            if left_mark == right_mark + 1:
                ...
                yield
                self.list[start], self.list[right_mark] =\
                                        self.list[right_mark], self.list[start]
                self.displayed_list.itemconfig(
                        self.cells[start], text=self.list[start],
                        fill=self.nonmoving_item_colour
                                              )
                self.displayed_list.itemconfig(self.cells[right_mark],
                                               text=self.list[right_mark],
                                               fill=self.in_place_item_colour
                                              )
                yield
                return
            ...
            yield
            self.list[left_mark], self.list[right_mark] =\
                            self.list[right_mark], self.list[left_mark]
            self.displayed_list.itemconfig(self.cells[left_mark],
                                           text=self.list[left_mark],
                                           fill=self.nonmoving_item_colour
                                          )
            self.displayed_list.itemconfig(self.cells[right_mark],
                                           text=self.list[right_mark],
    ...
    def trace_heap_sort(self):
        ...
        if self.length > 2:
            ...
            self.displayed_list.itemconfig(self.cells[2],
                                           fill=self.moving_item_colour,
                                           font=('normal', 0, 'bold')
                                          )
            yield
        ...
        if self.list[0] > self.list[1]:
            ...
            self.displayed_list.itemconfig(self.cells[1],
                                           fill=self.moving_item_colour,
                                           font=('normal', 0, 'bold')
                                          )
            yield
    ...
    def trace_bubble_down(self, i, length):
        while True:
            ...
            if index_of_greatest_child < length - 1:
                self.displayed_list.itemconfig(
                        self.cells[index_of_greatest_child],
                        fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                              )
                self.displayed_list.itemconfig(
                        self.cells[index_of_greatest_child + 1],
                        fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                              )
                yield
                if self.list[index_of_greatest_child] <\
                                    self.list[index_of_greatest_child + 1]:
                    self.displayed_list.itemconfig(
                            self.cells[index_of_greatest_child],
                            fill=self.live_item_colour,
                            font=('normal', 0, 'normal')
            ...
            if self.list[i] >= self.list[index_of_greatest_child]:
                ...
                yield
                self.list[i], self.list[index_of_greatest_child] =\
                            self.list[index_of_greatest_child], self.list[i]
                self.displayed_list.itemconfig(
                        self.cells[i], text=self.list[i],
                        fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'normal')
    ...
    def trace_batcher_sort(self):
        if self.length == 2:
            if self.list[0] > self.list[1]:
                ...
                self.displayed_list.itemconfig(
                        self.cells[0], fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                              )
                self.displayed_list.itemconfig(
                        self.cells[1], fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                              )
                yield
            self.displayed_list.itemconfig(self.cells[0], text=self.list[0],
                    ...
                    if self.list[top + i] > self.list[top + i + half_size]:
                        if half_size * 2 == self.length\
                           and top == 0 and i == 0:
                            ...
                            self.displayed_list.itemconfig(
                                    self.cells[top + i + half_size],
                                    fill=self.moving_item_colour,
                                    font=('normal', 0, 'bold')
                                                          )
                            yield
                            self.list[top + i],\
                                    self.list[top + i + half_size] =\
                                self.list[top + i + half_size],\
                                        self.list[top + i]
                            self.displayed_list.itemconfig(
                                    self.cells[top + i],
                        ...
                        elif half_size * 2 == self.length and top == 0\
                             and i + 1 == half_size:
                            self.displayed_list.itemconfig(
                                    self.cells[top + i],
                                    fill=self.moving_item_colour,
                        ...
                        if half_size * 2 == self.length and top == 0\
                           and i == 0:
                            self.displayed_list.itemconfig(
                                    self.cells[top + i],
                                    fill=self.nonmoving_item_colour,
                ...
                for group in range(self.length // double_span):
                    ...
                    for i in range(span):
                        if self.list[top + i] > self.list[top + i + span]:
                            ...
                            self.displayed_list.itemconfig(
                                    self.cells[top + i + span],
                                    fill=self.moving_item_colour,
                                    font=('normal', 0, 'bold')
                                                          )
                            yield
                            self.list[top + i], self.list[top + i + span] =\
                                  self.list[top + i + span], self.list[top + i]
